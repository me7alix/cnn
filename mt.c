...
void mt_learn() {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        *thread_num_ptr = i;
        if (pthread_create(&threads[i], NULL, thread_function, thread_num_ptr) != 0) {
    ...
    while(!WindowShouldClose()){
        ...
        if(IsMouseButtonDown(MOUSE_LEFT_BUTTON)){
            ...
            if (x >= 0 && x < tiles && y >= 0 && y < tiles) {
                float b = 0.4;
                pixels[y][x] += b;
                if(x > 0) pixels[y][x - 1] += b/2.0;
                if(x < tiles-1) pixels[y][x + 1] += b/2.0;
                if(y > 0) pixels[y - 1][x] += b/2.0;
                if(y < tiles-1) pixels[y + 1][x] += b/2.0;
            }
